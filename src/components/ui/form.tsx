  ...
  if (!body) {
    return (
      <div
        ...
        id={formMessageId}
        className={cn('text-sm min-h-5', className)}
        {...props}
      />
    );
  }
...
const FluidFormElement = React.forwardRef<HTMLFormElement, React.ComponentPropsWithoutRef<'form'>>(
  ({ className, ...props }, ref) => {
    return <form ref={ref} className={cn('flex flex-col gap-4', className)} {...props} />;
  },
);

FluidFormElement.displayName = 'FluidFormElement';
