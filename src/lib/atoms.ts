...
import type { Bid, Collection, User } from '@/types'
...
// Derived atoms
export const openCollectionsAtom = atom((get) => get(collectionsAtom).filter((c) => c.status === 'open'))
...
export const sortedCollectionsAtom = atom((get) => {
	...
	return collections.slice().sort((a, b) => compareAsc(parseISO(a.updatedAt), parseISO(b.updatedAt)))
})
